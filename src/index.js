 ...
 */
function computeOutputSize(input_size, kernel_height, kernel_width, padding, dilation, stride) {
	return {
		output_height : Math.floor(
			(input_size + 2 * padding - dilation * (kernel_height - 1) - 1) / stride + 1
		),
		output_width  : Math.floor(
			(input_size + 2 * padding - dilation * (kernel_width - 1) - 1) / stride + 1
		),
	};
}
 ...
 */
function paramsOK(input_size, kernel_height, kernel_width, padding, dilation, stride) {
	const { output_height, output_width } = computeOutputSize(
		input_size,
		kernel_height,
		kernel_width,
		padding,
...
class App extends React.Component {
	...
	render() {
		...
		const kernel_height = this.state.kernel_height;
		const kernel_width = this.state.kernel_width;
		...
		const output = array2d(output_height, output_width, (i, j) =>
			array2d(kernel_height, kernel_width)
		);

		for (let h_out = 0; h_out < output_height; h_out++) {
			for (let w_out = 0; w_out < output_width; w_out++) {
				for (let h_kern = 0; h_kern < kernel_height; h_kern++) {
					for (let w_kern = 0; w_kern < kernel_width; w_kern++) {
		...
		return (
			<div>
				...
				<p>
					This interactive visualization demonstrates how various convolution parameters affect
					shapes and data dependencies between the input, weight and output matrices. Hovering over
					an input/output will highlight the corresponding output/input, while hovering over an
					weight will highlight which inputs were multiplied into that weight to compute an output.
					(Strictly speaking, the operation visualized here is a <em>correlation</em>, not a
					convolution, as a true convolution flips its weights before performing a correlation.
					However, most deep learning frameworks still call these convolutions, and in the end it's
				...
				<form className="form">
					<fieldset>
						...
						<Slider
							min={minWhile(max_input_size, 1, (x) =>
								paramsOK(x, kernel_height, kernel_width, padding, dilation, stride)
							)}
							max={max_input_size}
							value={input_size}
					...
					<fieldset>
						<legend>Kernel Height:</legend>
						<Slider
							min="1"
							max={maxWhile(1, 100, (x) =>
								paramsOK(input_size, x, kernel_width, padding, dilation, stride)
							)}
							value={kernel_height}
							onChange={onChange('kernel_height')}
						/>
					</fieldset>
					<fieldset>
						<legend>Kernel Width:</legend>
						<Slider
							min="1"
							max={maxWhile(1, 100, (x) =>
								paramsOK(input_size, kernel_height, x, padding, dilation, stride)
							)}
							value={kernel_width}
							onChange={onChange('kernel_width')}
						/>
					</fieldset>
					<fieldset>
						<legend>Padding:</legend>
						<Slider
							min={minWhile(dilation * (Math.min(kernel_height, kernel_width) - 1), 0, (x) =>
								paramsOK(input_size, kernel_height, kernel_width, x, dilation, stride)
							)}
							max={dilation * (Math.min(kernel_height, kernel_width) - 1)}
							value={padding}
					...
					<fieldset>
						...
						<Slider
							min="1"
							max={maxWhile(1, 100, (x) =>
								paramsOK(input_size, kernel_height, kernel_width, padding, x, stride)
							)}
							value={dilation}
							onChange={onChange('dilation')}
							disabled={Math.min(kernel_height, kernel_width) === 1}
						/>
					</fieldset>
					<fieldset>
						...
						<Slider
							min="1"
							max={Math.max(input_size - dilation * (Math.min(kernel_height, kernel_width) - 1), 1)}
							value={stride}
...
class Viewport extends React.Component {
	...
	render() {
		...
		const kernel_width = this.props.kernel_width;
		const kernel_height = this.props.kernel_height;
		//const output_size = this.props.output_size;
		const output_height = this.props.output_height;
		const output_width = this.props.output_width;
		...
		console.log('OUTPUT HxW', output_height, output_width);
		...
		console.log('OUT HxW', animatedH, animatedW);
		...
		if (hoverOver === 'input') {
			hoverOver = 'output';
			hoverH = Math.min(Math.floor(hoverH / stride), output_height - 1);
			hoverW = Math.min(Math.floor(hoverW / stride), output_width - 1);
		}
		...
		const scale_size = Math.max(kernel_height, kernel_width);
		const xScale = d3
			.scaleSequential(d3.interpolateLab('#d7191c', '#2c7bb6'))
			.domain([ -1, scale_size ]);
		const yScale = d3
			.scaleSequential(d3.interpolateLab('#d7191c', d3.color('#1a9641').brighter(1)))
			.domain([ -1, scale_size ]);
		...
		function compute_input_multiplies_with_weight(hoverH, hoverW) {
			...
			for (let h_weight = 0; h_weight < kernel_height; h_weight++) {
				for (let w_weight = 0; w_weight < kernel_width; w_weight++) {
					// console.log('KHeight', kernel_height, 'KWidth', kernel_width);
					// console.log(hoverH, hoverW, h_weight, w_weight);
		...
		return (
			<div className="viewport">
				<div className="grid-container">
					...
					<Grid
						height={input_size + 2 * padding}
						width={input_size + 2 * padding}
